...
def bop(sax_words):
    ...
    for word in range(len(sax_words)):
        if not sax_words[word] in dict:
            dict[sax_words[word]] = 0
	#Lizhi: Based on the paper "OS-level side channel ... by XiaoFeng Wang", to avoid over-counting
        #trivial matches [46], i.e., similar words that are neighbors in the original SAX strings, 
        #we count only the first occurrence of each word.
        if cmp(sax_words[word], sax_words[word-1]) != 0:
            dict[sax_words[word]] = dict[sax_words[word]] + 1
        
    return dict

#lizhi test
arr = [2.02, 2.33, 2.99, 6.85, 9.20, 8.80, 7.50, 6.00, 5.85, 3.85, 4.85, 3.85, 2.22, 1.45, 1.34]
l= sax(arr,5,None,4)
print l
s = sax_words(standardize(arr),6,3,2,None,4,True)
print(s)
m = bop(s)
print m
...
arr1 = [1.0, 1.0, 2.0, 2.0, -2.0, -3.0, 3.0,3.0,2.5,2.5,-5.0,-2.0,-3.0,-2.4,-6.0,-6.9,-5.0,-6.0,-5.1,-5.6]
l1 = sax(arr1,10,None,2)
print l1
s1 = sax_words(standardize(arr1),4,2,2,None,2,True)
print(s1)
m1 = bop(s1)
print m1
